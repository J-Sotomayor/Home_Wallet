...
|---|---|---|---|
| 1 | Individual / Pareja / Familia / Grupo | Implementado | Existen los cuatro tipos. Individual tiene un único integrante y oculta invitaciones, integrantes, división, deudas y filtros colaborativos. Para evolucionar sin exponer el historial personal se crea o se une a un espacio compartido separado; el espacio Individual y sus movimientos permanecen privados. |
| 2 | Invitaciones | Implementado con protección de servidor | Generación por código/QR, token aleatorio con hash, vencimiento de 15 minutos y uso único. UI y Cloud Functions impiden invitar desde Individual, aceptar hacia Individual, reutilizar o aceptar tokens vencidos, y controlan pertenencia/cupos. |
...
| 18 | Firebase / Firestore | Implementado y revisado estáticamente | Authentication, Firestore, Storage, Functions y FCM están conectados. Se revisaron colecciones/subcolecciones, índices, reglas, roles, `createdBy`, `householdId`, timestamps, cifrado, transacciones y manejo de errores. Los secretos de usuario no se guardan en el repositorio. |
...
| 20 | Seguridad y privacidad | Implementado | Aislamiento por espacio y rol en Firestore/Storage, contenido financiero cifrado antes de sincronizar, funciones privilegiadas para invitaciones/roles/borrado, tokens FCM protegidos, archivos bancarios locales y texto legal coherente con el tratamiento de datos. |
...
| 24 | Rendimiento | Sin métricas inventadas | No se encontró una medición reproducible de P95 ni una prueba formal de 1000 movimientos. La app limita consultas a 1000 y archivos importados a 500, pero esos límites no equivalen a una métrica. La tesis debe omitir o reformular cualquier cifra hasta medirla. |

## Validaciones locales ejecutadas

- `flutter analyze`: sin problemas.
- `flutter test`: 14 pruebas aprobadas, incluidas tema claro tras verificación, cuatro tipos/roles, división proporcional, comparación mensual, deduplicación y exportación XLSX/PDF/CSV.
- `npm run build` en `functions`: compilación TypeScript aprobada.
- Compilación e instalación Android: debe repetirse como última validación después de cada cambio antes de entregar el APK.
