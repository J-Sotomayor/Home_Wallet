...
| 18 | Firebase / Firestore | Implementado y revisado estáticamente | Authentication, Firestore, Storage, Functions y FCM están conectados. Se revisaron colecciones/subcolecciones, índices, reglas, roles, `createdBy`, `householdId`, timestamps, cifrado, transacciones y manejo de errores. La inicialización y exigencia de App Check permanecen desactivadas durante las instalaciones directas en dispositivos físicos para no bloquear teléfonos legítimos; deben activarse de forma controlada y revisar sus métricas al publicar por Google Play. Los secretos de usuario no se guardan en el repositorio. |
