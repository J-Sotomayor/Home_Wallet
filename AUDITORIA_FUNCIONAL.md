...
| 7 | Dividir gasto y deudas | Implementado | División igualitaria, porcentual, personalizada y proporcional a los ingresos netos mensuales cifrados que cada integrante registra desde Inicio; participantes, pagador, porción, saldo, deuda, liquidación y redondeo al centavo. Es independiente de registrar un movimiento y no aparece en Individual. |
...
| 14 | Notificaciones | Implementado; entrega externa pendiente de validación final | FCM y notificaciones locales cubren ingresos, gastos, ahorros/aportes a metas, divisiones, planes, invitación aceptada, cambio de rol, retiro y recurrencias. Los textos se adaptan a Individual, Pareja, Familia y Grupo; una tarea en segundo plano recuerda revisar el espacio lunes y jueves. Android está compilado; en iOS la recepción física depende de validar permisos, token APNs/FCM y la clave APNs cargada en Firebase. |
| 15 | Bloqueo de HomeWallet | Implementado en código compartido | Activación/desactivación, autenticación biométrica o credencial segura configurada en Android/iOS, bloqueo al salir y solicitud al volver. La app no almacena la biometría. Android está validado; Face ID/Touch ID debe comprobarse en Simulator y dispositivo Apple. |
...
| 17 | Android e iOS | Android validado; iOS preparado y pendiente de validación en Mac | Android conserva su configuración y flujo de compilación. iOS incluye Podfile, Firebase, Google Sign-In, permisos, capacidades y un script reproducible de preparación; por limitación del entorno Windows todavía requiere compilarse con Xcode y probarse en Simulator/iPhone antes de declararlo validado. |
...
- `flutter test`: 19 pruebas aprobadas, incluidas tema claro tras verificación, cuatro tipos/roles, Pareja completa sin nueva invitación, ingresos mensuales cifrados, división proporcional, comparación mensual, historial sin corte arbitrario, deduplicación y exportación XLSX/PDF/CSV.
- `npm run build` en `functions`: compilación TypeScript aprobada.
- Firebase Emulator: 6 pruebas de reglas aprobadas (aislamiento, Individual sin divisiones, Jr. de solo lectura, selector activo autorizado, perfil/ingreso propio y bloqueo directo de copias de clave) y 5 pruebas de Functions aprobadas (Individual único, más de cinco espacios, rol/revocación, concurrencia de Pareja, conservación de membresías y recuperación autorizada de clave).
...
- Android release: APK `1.0.7+8` compilado y firma v1/v2 verificada. La instalación y los flujos entre dos teléfonos quedan como prueba manual final.
